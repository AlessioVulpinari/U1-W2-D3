...
for (let i = 0; i < starWarsCharacters.length; i++) {
  switch (starWarsCharacters[i].eye_color) {
    ...
    default:
      console.log("Colore non presente: " + starWarsCharacters[i].eye_color);
  }
}
...
for (let i = 0; i < starWarsCharacters.length; i++) {
  for (let j = 0; j < femaleCharacters.length; j++) {
    if (starWarsCharacters[i] === femaleCharacters[j]) {
...
function getRandomValue(min, max) {
  const minCeiled = Math.ceil(min); // minimo valore incluso
  const maxFloored = Math.floor(max); // massimo valore escluso
  return Math.floor(Math.random() * (maxFloored - minCeiled) + minCeiled);
}

console.log(starWarsCharacters[getRandomValue(0, starWarsCharacters.length)]);
